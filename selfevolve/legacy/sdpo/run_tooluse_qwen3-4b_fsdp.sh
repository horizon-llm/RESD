...
"$PYTHON" - <<'PY'
import inspect
import selfevolve.sdpo as sdpo
from selfevolve.sdpo.workers.config.actor import SelfDistillationConfig
from selfevolve.sdpo.workers.rollout.vllm_rollout import vllm_async_server

sdpo_path = inspect.getfile(sdpo)
vllm_worker_path = inspect.getfile(vllm_async_server)
print("[sanity] selfevolve.sdpo imported from:", sdpo_path)
print("[sanity] SelfDistillationConfig imported:", SelfDistillationConfig)
print("[sanity] vLLM worker module imported from:", vllm_worker_path)

if "/selfevolve/sdpo/" not in sdpo_path or "/selfevolve/sdpo/" not in vllm_worker_path:
    raise RuntimeError(
        "Sanity check failed: expected selfevolve/sdpo path but got different module path. "
        f"sdpo={sdpo_path}, vllm_worker={vllm_worker_path}"
    )
PY
...
exp_name="qwen3_4b_fsdp_trbs${TRAIN_BATCH_SIZE}_rbs${ROLLOUT_BATCH_SIZE}_maxlen${MAX_RESPONSE_LENGTH}_ep${NUM_EPOCHS}_alpha${ALPHA}_lambda${LAMBDA}_lr${LR}_clip${CLIP_ADV_HIGH}_dross${DONTS_REPROMPT_ON_SELF_SUCCESS}_ema${EMA_WEIGHT}_reward_count"
...
DATA=(
    ...
    data.shuffle=False
    custom_reward_function.path=selfevolve/sdpo/utils/reward_score/feedback/tooluse.py
...
"$PYTHON" -m selfevolve.sdpo.trainer.main_ppo \
