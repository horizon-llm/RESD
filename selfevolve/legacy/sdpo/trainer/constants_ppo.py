...
def get_ppo_ray_runtime_env():
    ...
    # Propagate PYTHONPATH to Ray workers so they resolve the correct verl package.
    # Also propagate PYTHONSAFEPATH to prevent workers from prepending CWD to
    # sys.path, which would cause the project-root verl to shadow the one on
    # PYTHONPATH (e.g. selfevolve/sdpo/verl).
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        runtime_env["env_vars"]["PYTHONPATH"] = pythonpath
    pythonsafepath = os.environ.get("PYTHONSAFEPATH")
    if pythonsafepath:
        runtime_env["env_vars"]["PYTHONSAFEPATH"] = pythonsafepath
    return runtime_env
