...
from ..experimental.dataset.sampler import AbstractSampler
from ..trainer.constants_ppo import get_ppo_ray_runtime_env
from ..trainer.ppo.ray_trainer import RayPPOTrainer
from ..trainer.ppo.reward import load_reward_manager
from ..trainer.ppo.utils import need_critic, need_reference_policy
from ..utils.config import validate_config
from ..utils.device import auto_set_device, is_cuda_available
from ..utils.import_utils import load_extern_object
...
def run_ppo(config, task_runner_class=None) -> None:
    ...
    if (
        ...
        from ..utils.import_utils import is_nvtx_available
...
class TaskRunner:
    ...
    def add_actor_rollout_worker(self, config):
        ...
        from ..trainer.ppo.ray_trainer import Role
        ...
        if use_legacy_worker_impl == "disable":
            from ..workers.engine_workers import ActorRolloutRefWorker
        ...
        if config.actor_rollout_ref.actor.strategy in {"fsdp", "fsdp2"}:
            from ..workers.fsdp_workers import AsyncActorRolloutRefWorker

            actor_rollout_cls = AsyncActorRolloutRefWorker
            ray_worker_group_cls = RayWorkerGroup

        elif config.actor_rollout_ref.actor.strategy == "megatron":
            from ..workers.megatron_workers import AsyncActorRolloutRefWorker
    ...
    def add_critic_worker(self, config):
        ...
        if config.critic.strategy in {"fsdp", "fsdp2"}:
            if use_legacy_worker_impl in ["auto", "enable"]:
                from ..workers.fsdp_workers import CriticWorker
            elif use_legacy_worker_impl == "disable":
                # we don't need to specialize critic worker. Just use TrainingWorker
                from ..workers.engine_workers import TrainingWorker
    ...
    def init_resource_pool_mgr(self, config):
        ...
        from ..trainer.ppo.ray_trainer import ResourcePoolManager
        ...
        if config.reward_model.enable:
            ...
            if use_legacy_worker_impl in ["auto", "enable", "disable"]:
                if config.reward_model.strategy in {"fsdp", "fsdp2"}:
                    ...
                    from ..workers.megatron_workers import RewardModelWorker
                else:
                    raise NotImplementedError
            # elif use_legacy_worker_impl == "disable":
    ...
    def run(self, config):
        ...
        from ..utils.dataset.rl_dataset import collate_fn
...
def create_rl_dataset(data_paths, data_config, tokenizer, processor, is_train=True, max_samples: int = -1):
    ...
    from ..utils.dataset.rl_dataset import get_dataset_class
