...
CONFIG_NAME="grpo"
NUM_DATA=${NUM_DATA:--1}
ALGO_CATEGORY=${ALGO_CATEGORY:-"meet_in_the_middle"}

python selfevolve/sdpo_fewshot/data/format/competitioncode.py \
    --train_data_source "competitioncode/${ALGO_CATEGORY}" \
    --test_data_source "competitioncode/${ALGO_CATEGORY}_seed" \
    --num_data ${NUM_DATA} \
    --data_source_suffix "${ALGO_CATEGORY}"

train_path=selfevolve/sdpo_fewshot/datasets/competitioncode_${ALGO_CATEGORY}/train_${NUM_DATA}.parquet
val_path=selfevolve/sdpo_fewshot/datasets/competitioncode_${ALGO_CATEGORY}/test.parquet

########################### Quick Config ###########################

TASK=competitioncode_${ALGO_CATEGORY}
export TASK
...
project_name='grpo_stream_competitioncode'
...
exp_name="qwen3_4b_${FSDP_STRATEGY}_${ALGO_CATEGORY}"
_add ndata   "$NUM_DATA"
...
DATA=(
    ...
    custom_reward_function.path=selfevolve/sdpo_fewshot/feedback/competitioncode.py
