...
class RolloutManager(BaseBenchmark):
    ...
    def load_epoch_data(
        self, epoch: int, shuffle: bool = True, truncate: int = None, cache_dim: str = "epoch"
    ) -> None:
        """Prepare data for a specific epoch."""
        epoch_data = self.dataset.load_epoch_data(epoch, shuffle=shuffle, truncate=truncate, cache_dim=cache_dim)
        self.curr_epoch = epoch
        self.curr_cache_dim = cache_dim
        return epoch_data
    ...
    async def _run_batch(
        ...
        if not stat:
            logger.warning("No statistics generated for current batch. Returning empty metrics.")
            return rollouts, {}
    ...
    def _get_batch_samples(self, batch_idx: int | None = None, stage: str = None) -> list[EvaluationSample]:
        ...
        samples = self.dataset.get_batch_samples(
            ...
            cache_dim=getattr(self, "curr_cache_dim", "epoch"),
        )
        return samples
