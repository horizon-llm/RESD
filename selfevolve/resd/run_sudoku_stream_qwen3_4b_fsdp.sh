...
if [[ "$USE_HARD_DATA" == "True" ]]; then
    python selfevolve/resd/preprocess.py --truncate_parquet selfevolve/resd/datasets/sudoku/train_hard.parquet --num_data $NUM_DATA
    train_path=selfevolve/resd/datasets/sudoku/train_hard_${NUM_DATA}.parquet
else
    python selfevolve/resd/preprocess.py --truncate_parquet selfevolve/resd/datasets/sudoku --num_data $NUM_DATA
    train_path=selfevolve/resd/datasets/sudoku/train_${NUM_DATA}.parquet
fi

val_path=selfevolve/resd/datasets/sudoku/test.parquet
...
cu_teacher_prompt_file=${cu_teacher_prompt_file:-"selfevolve/resd/context_updater/prompts/sudoku_generator_v1.txt"} # path to a .txt file with custom context-updater teacher prompt; null uses built-in default
...
_add maxpl   "$MAX_PROMPT_LENGTH"          65536
_add maxlen  "$MAX_RESPONSE_LENGTH"        32768
_add maxrp   "$MAX_REPROMPT_LENGTH"        65536
