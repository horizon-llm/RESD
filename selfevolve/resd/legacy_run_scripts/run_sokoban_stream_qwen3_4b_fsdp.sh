...
if [[ "$USE_HARD_DATA" == "True" ]]; then
    python selfevolve/resd/data/format/sokoban.py --input_parquet selfevolve/resd/datasets/sokoban/train_hard.parquet --num_data $NUM_DATA $SORT_FLAG
    train_path=selfevolve/resd/datasets/sokoban/train_hard_${NUM_DATA}.parquet
else
    python selfevolve/resd/data/format/sokoban.py --input_parquet selfevolve/resd/datasets/sokoban --num_data $NUM_DATA $SORT_FLAG
    train_path=selfevolve/resd/datasets/sokoban/train_${NUM_DATA}.parquet
fi

val_path=selfevolve/resd/datasets/sokoban/test.parquet
...
reflector_prompt_file=${reflector_prompt_file:-"selfevolve/resd/context_updater/prompts/sokoban_reflector_v2.txt"} # path to a .txt file with custom reflector prompt; null uses built-in default
curator_prompt_file=${curator_prompt_file:-null} # path to a .txt file with custom curator prompt; null uses built-in default
cu_teacher_prompt_file=${cu_teacher_prompt_file:-"selfevolve/resd/context_updater/prompts/sokoban_generator_v1.txt"} # path to a .txt file with custom context-updater teacher prompt; null uses built-in default
...
DATA=(
    ...
    custom_reward_function.path=selfevolve/resd/feedback/reasoning_gym_games/__init__.py
