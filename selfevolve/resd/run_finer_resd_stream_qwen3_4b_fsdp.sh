...
success_rate_weighting=${success_rate_weighting:-True} # whether to weight distillation loss by group success rate
...
# === context updater ===
use_context_updater=${use_context_updater:-True}
...
concise_method=${concise_method:-"staleness"} # method for concising context, choose from "reset", "prioritized" and "staleness"
...
use_solution_in_teacher_prompt=${use_solution_in_teacher_prompt:-True} # whether to include successful solutions in the teacher prompt; requires {solution} placeholder in template
...
cu_teacher_prompt_file=${cu_teacher_prompt_file:-selfevolve/resd/context_updater/prompts/finer_generator_v4.txt} # path to a .txt file with custom context-updater teacher prompt; null uses built-in default
# === stream trainer ===
...
reward_function=${reward_function:-"compute_score_count"} # which reward function to use for distillation feedback; must be a function defined in selfevolve/resd/feedback/finer.py

project_name='resd_stream_finer'
...
_add maxlen  "$MAX_RESPONSE_LENGTH"        12288
...
_add sct      "$success_reward_threshold"  1.0
