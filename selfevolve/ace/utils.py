...
def sanitize_model_name(model_name: str) -> str:
    """Convert a model name to a path-safe string.

    Uses the HuggingFace convention of replacing '/' with '--'.
    E.g. 'Qwen/Qwen3-30B-A3B-Thinking-2507' -> 'Qwen--Qwen3-30B-A3B-Thinking-2507'
    """
...
def setup_paths(
    ...
    """Setup logging paths and directories for an ACE run.

    Args:
        save_dir: Base directory for saving results.
        task_name: Name of the task (e.g. 'finer').
        mode: One of 'offline', 'online', or 'eval_only'.
        generator_model: Generator model name (will be sanitized for the path).

    Returns:
        For eval_only mode: (save_path, log_dir)
        For other modes:    (save_path, usage_log_path, playbook_dir, log_dir)
    """
